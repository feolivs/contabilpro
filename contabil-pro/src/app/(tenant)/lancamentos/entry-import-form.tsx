  ...
  return (
    <form action={action} className='grid gap-4 rounded-lg border bg-card p-6 shadow-sm'>
      <div>
        ...
        <p className='text-sm text-muted-foreground'>Cada linha deve conter client_id, account_id, description, amount, type, date e document_id (opcional).</p>
      </div>

      <div className='grid gap-2'>
        <Label htmlFor='file'>Arquivo CSV</Label>
        <Input id='file' name='file' type='file' accept='.csv' required disabled={isPending} />
        <p className='text-xs text-muted-foreground'>Campos type aceitam debit ou credit. Datas devem estar no formato ISO (YYYY-MM-DD).</p>
      </div>
