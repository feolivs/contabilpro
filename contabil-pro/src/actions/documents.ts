...
// ============================================
export async function getDocumentAIInsights(documentId: string): Promise<ActionResponse<any[]>> {
  try {
    const session = await requireAuth()
    const supabase = await createServerClient()
...
export async function translateToAccountingLanguage(
  ...
  try {
    ...
    switch (documentType) {
      ...
      default:
        explanation =
          'Este documento contém informações que podem ser relevantes para a contabilidade. '
        explanation +=
          'Recomenda-se análise detalhada para determinar o tratamento contábil adequado.'
